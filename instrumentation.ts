export async function register() {
  if (process.env.NEXT_RUNTIME === "nodejs") {
    try {
      ...
      // Loga o erro mas NÃO lança — impede que uma falha de DB no startup
      // derrube toda a função serverless e cause HTML 500 no login.
      console.error("[DB] Falha na inicialização do schema (não fatal):", err);
    }
  }
}
